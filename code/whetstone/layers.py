...
class Spiking(Layer):
    ...
    def sharpen(self, amount=0.01):
        ...
        """
        K.set_value(self.sharpness, min(max(K.get_value(self.sharpness) +
                    amount, Spiking.sharpen_start_limit), Spiking.sharpen_end_limit))
...
def key_generator(num_classes: int, width: int,
                  sparsity: float = 0.1, overlapping: bool = True) -> np.ndarray:
...
spikingLayersDict = dict([(cls.__name__, cls)
                         for cls in vars()['Spiking'].__subclasses__()])
