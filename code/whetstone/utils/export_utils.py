...
def merge_batchnorm(keras_preactivation_layer, keras_batchnorm_layer):
    ...
    # Arguments
        ...
        keras_batchnorm_layer: The batch normalization layer to be merged into the
                               preactivation layer.

    # Returns
        new_layer, (new_weights, new_biases) : Where new_layer is a keras layer of the
        same configuration as keras_preactivation_layer, and (new_weights, new_biases) are
        the updated weights and biases for the new_layer to be used with
        new_layer.set_weights(((new_weights, new_biases))) after the new model is built.
    """
    assert type(
        keras_batchnorm_layer) == keras.layers.normalization.BatchNormalization
    ...
    # TODO There may be a more canonical way to do this.
    new_layer = type(keras_preactivation_layer)(**config)
...
def copy_remove_batchnorm(keras_sequential_model):
    """Make a functionally equivalent copy of a net with the BatchNormalization layers
    removed.

    Given a keras Sequential model, returns an equivalent Sequential model without
    batchnorm layers.
