...
from .utils.layer_utils import \
    get_spiking_layer_indices, set_layer_sharpness, set_model_sharpness
import os
import sys
import json
import pickle
...
class Sharpener(Callback):
    ...
    # Arguments
        bottom_up: Boolean, if ``True``, sharpens one layer at a time,
                   sequentially, starting with the first. If ``False``, sharpens all
                   layers uniformly.
    ...
    def set_model_sharpness(self, value):
        ...
        # Arguments
            value: Float, value between 0.0 and 1.0 inclusive that specifies the sharpness
                   of the model.
        """
        values = set_model_sharpness(
            model=self.model, value=value, bottom_up=self.bottom_up)
        self.sharpness = values
...
class SimpleSharpener(Sharpener):
    ...
    def get_config(self):
        config = {'epochs': self.epochs,
                  'start_epoch': self.start_epoch, 'steps': self.steps}
        base_config = super(SimpleSharpener, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))

    def on_train_begin(self, logs=None):
        super(SimpleSharpener, self).on_train_begin(logs)
        self.total_steps = (self.steps * self._num_spiking_layers()
                            ) if self.bottom_up else self.steps
...
class ScheduledSharpener(Sharpener):
    ...
    # Arguments
        schedule: List of tuples of the form
            [(start_epoch, stop_epoch), (start_epoch, stop_epoch), ...]
    ...
    def __init__(self, schedule=None, num_layers=None, start=None, duration=None, intermission=None, **kwargs):
        ...
        if schedule is None:
            schedule = self.gen_schedule(
                num_layers, start, duration, intermission)
        else:
            assert type(schedule) is list
            assert all([type(st) is int and type(
                sp) is int for (st, sp) in schedule])
    ...
    def _init_schedule(self):
        ...
        self.incs = [1.0/(((stop-start)*self.num_batches)-1)
...
class RLSharpener(Sharpener):
    ...
    def __init__(self, start_step, layer_duration, **kwargs):
        ...
        # how many steps to sharpen each layer over.
    ...
    def on_episode_end(self, episode, logs):
        ...
        # ensures training halts when all layers sharpened.
        super(RLSharpener, self).on_epoch_end(1, None)

    def _perform_sharpening(self, step):
        model_sharpness = min(1.0, max(0.0, int(step) - self.start_step) /
                              float(self.layer_duration) / float(self._num_spiking_layers()))
...
class AdaptiveSharpener(Sharpener):
    ...
    def __init__(self, min_init_epochs=10,
        ...
        assert type(
            first_layer_relative_rate) is float and first_layer_relative_rate > 0.0
    ...
    def on_train_begin(self, logs=None):
        ...
        # number of epochs since the loss improved significantly.
    ...
    def _perform_sharpening(self, logs=None):
        unfinished_layers = [idx for idx,
                             s in enumerate(self.sharpness) if s < 1.0]
        if len(unfinished_layers) > 0:
            if self.bottom_up:
                if not self.wait:
                    ...
                    self.sharpness[sharpen_idx] = min(
                        1.0, self.sharpness[sharpen_idx] + sharpen_amount)
                ...
                if self.sharpness[0] >= self.critical:
                    sharpen_amount = self.cz_rate
                sharpen_amount *= (1.0/float(self.batches_per_epoch))
                new_uniform_sharpness = min(
                    1.0, self.sharpness[0] + sharpen_amount)
                if 1.0 - new_uniform_sharpness < 0.000001:
                    new_uniform_sharpness = 1.0
                self.sharpness = [
                    new_uniform_sharpness for _ in range(len(self.sharpness))]
            self.set_layer_sharpness(values=self.sharpness)
        else:
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        percent_change = (
            logs['loss'] - self.reference_loss) / self.reference_loss
...
class WhetstoneLogger(Callback):
    ...
    def __init__(self, logdir,
        ...
        assert test_set is None or (
            type(test_set) is tuple and len(test_set) == 2)
    ...
    def on_train_begin(self, logs=None):
        ...
        try:
            environ_info['whetstone_version'] = \
                pkg_resources.get_distribution('whetstone').version
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        logs_ = {'train_loss': logs['loss'],
                 'train_accuracy': logs['accuracy']}
        if self.sharpener is not None:
            logs_['sharpness'] = self.sharpener.sharpness
        if self.test_set is not None:
            (x_test, y_test) = self.test_set
            logs_['test_loss'], logs_['test_accuracy'] = self.model.evaluate(
                x_test, y_test, verbose=0)[0:2]
            if self.sharpener is not None:
                self.sharpener.set_layer_sharpness(
                    values=[1.0 for _ in logs_['sharpness']])
                logs_['test_loss_spiking'], logs_[
                    'test_accuracy_spiking'] = self.model.evaluate(x_test, y_test, verbose=0)[0:2]
                self.sharpener.set_layer_sharpness(
                    values=logs_['sharpness'])  # restore
        log_path = os.path.join(epoch_path, 'log.json')
        with open(log_path, 'w') as f:
            json.dump(logs_, f, indent=4)
        if self.log_weights:
            self.model.save(os.path.join(
                epoch_path, 'model_epoch_'+str(epoch)+'.h5'))
