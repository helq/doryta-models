...
def load_model(filepath):
    ...
    Loads and returns the Keras/Whetstone model from a .h5 file at ``filepath``, handling
    custom layer deserialization for you.

    # Arguments
        filepath: Path to Keras/Whetstone model which should be a .h5 file produced by
                  model.save(filepath).
...
def decode_from_key(key, input_vec):
    ...
    # return [1*(np.argmax(np.matmul(2*(1-input_vec),rescaled_key))==i)
    #         for i in range(0, key.shape[0])]
    return [1*(np.argmax(np.matmul(2*key-1, 2*input_vec-1)) == i)
            for i in range(0, key.shape[0])]
...
def set_model_sharpness(model, value, bottom_up):
    ...
    if bottom_up:
        if value == 1.0:  # this makes sure rounding errors don't prevent full sharpening at 1.0
            ...
            # for the layers already done sharpening.
            values = [1.0 for _ in range(num_fully_sharpened)]
            # for the layer that's currently undergoing sharpening.
            values.append(scaled_remainder)
            # for the layers that have not yet begun to sharpen.
            values.extend(
                [0.0 for _ in range(num_spiking_layers - num_fully_sharpened - 1)])
            set_layer_sharpness(model=model, values=values)
    else:  # uniform
